...
# tensorboard --logdir=1e-4:logs_rms10,2e-4:logs_rms10_1,3e-4:logs_rms10_2,5e-5:logs_rms10_3,1e-5:logs_rms10_4 --port 6006
...
def main():
    ...
    for i in range(4, tests.__len__()):
        print('--- testing: {} lrate'.format(tests[i]))

        model = build_model(y_test.shape[1])

        model = train(model, X_train, y_train, X_test, y_test, 'logs_rms10_{}'.format(i), tests[i])
