...
# tensorboard --logdir run0:logs_sgd10_0,run1:logs_sgd10_1,run2:logs_sgd10_2,run3:logs_sgd10_3,run4:logs_sgd10_4,run5:logs_sgd10_5,run6:logs_sgd10_6,run7:logs_sgd10_7 --port 6006
def train_sgd(model, X_train, y_train, X_test, y_test, log_dir, tests, epochs):
    ...
    model.fit(X_train, y_train, validation_data=(X_test, y_test), epochs=epochs, batch_size=64, callbacks=callbacks, verbose=verbose)
...
def main():
    ...
    for i in range(0, 1):
        ...
        model = train_sgd(model, X_train, y_train, X_test, y_test, 'logs_sgd100_{}'.format(i), tests[i], epochs)

        filename = 'out/sdg100_{}.h5'.format(i)
        save(model, filename)
    ...
    for i in range(0, 3):
        ...
        model = train_rms(model, X_train, y_train, X_test, y_test, 'logs_rms100_{}'.format(i), tests[i], epochs)

        evaluate(model, X_test, y_test)

        filename = 'out/rms100_{}.h5'.format(i)
        save(model, filename)
    ...
    for i in range(0, 3):
        ...
        model = train_adam(model, X_train, y_train, X_test, y_test, 'logs_adam100_{}'.format(i), tests[i], epochs)

        evaluate(model, X_test, y_test)

        filename = 'out/adam100_{}.h5'.format(i)
        save(model, filename)
